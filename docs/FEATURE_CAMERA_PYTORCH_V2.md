...
- 使用 ONNX Runtime 在端侧分析实时帧，组合人物/物体检测、人体 pose 关键点和 Places365 场景分类。
- 根据场景类别、障碍物、亮度、人物位置和三分线候选区域生成推荐站位。
- 根据传入的身高体重生成线条式 3D 虚拟人像，并在预览中按构图区域实时投影。
...
- `places365_resnet18.onnx`：Places365 场景分类。
...
- Places365 场景分类权重：`https://github.com/CSAILVision/places365`
...
- `CameraV2OnnxAnalyzer`：运行 ONNX Runtime session，解析检测、pose 和场景分类结果。
- `CameraV2CompositionEngine`：纯 Kotlin 构图规则，不依赖 Android UI。
- `VirtualHumanProjector`：根据身高体重和 pose 模板生成 3D 线框投影。
- `CameraV2Overlay`：绘制 3D 虚拟人像、构图区域、移动提示和模型状态。

## 测试

新增测试覆盖：

- `CameraV2CompositionEngineTest`：构图评分、暗光、场景偏好、模型缺失降级。
- `VirtualHumanProjectorTest`：不同身高体重下的人像比例和边界 clamp。
