...
class CompositionGuideEngine {
    ...
    fun buildGuide(
        ...
        if (pose == null) {
            return CameraGuideState(
                guideBounds = guideBounds,
                poseLines = poseLines,
                message = "实时检测中：未检测到人物，请移动相机对准人物",
                sceneQuality = CameraSceneQuality.NEEDS_MOVE,
                movementDirection = CameraMovementDirection.NONE
            )
        }

        if (pose.bounds.height > guideBounds.height * 1.15f) {
            return CameraGuideState(
                guideBounds = guideBounds,
                poseLines = poseLines,
                message = "实时检测：人物太近，请后退一点再拍",
                sceneQuality = CameraSceneQuality.NEEDS_MOVE,
                movementDirection = CameraMovementDirection.NONE
            )
        }

        if (pose.bounds.height < guideBounds.height * 0.45f) {
            return CameraGuideState(
                guideBounds = guideBounds,
                poseLines = poseLines,
                message = "实时检测：人物太远，请靠近一点再拍",
                sceneQuality = CameraSceneQuality.NEEDS_MOVE,
                movementDirection = CameraMovementDirection.NONE
            )
        }

        val horizontalOffset = pose.bounds.centerX - guideBounds.centerX
        ...
        val verticalOffset = pose.bounds.centerY - guideBounds.centerY
    ...
    private fun movementMessage(direction: CameraMovementDirection): String =
        when (direction) {
            ...
            CameraMovementDirection.NONE -> "场景有些杂乱，换个角度试试"
        }
